    ...
    for w in keras_model.weights:
        if "block" in w.name:
            ...
            tf_name = keras_name_to_tf_name_block(
                w.name, keras_block=keras_block, tf_block=tf_block, use_ema=use_ema, model_name_tf=model_name_tf
            )
        ...
        try:
            ...
            if any([x in w.name for x in ["top", "predictions", "probs"]]):
                warnings.warn("Fail to load top layer variable {}" "from {} because of {}.".format(w.name, tf_name, e))
            else:
...
def keras_name_to_tf_name_stem_top(keras_name, use_ema=True, model_name_tf="efficientnet-b0"):
    ...
    for bn_weights in ["beta", "gamma", "moving_mean", "moving_variance"]:
        tf_name = "{}/stem/tpu_batch_normalization/{}{}".format(model_name_tf, bn_weights, ema)
        stem_top_dict["stem_bn/{}:0".format(bn_weights)] = tf_name

    # top / head batch normalization
    for bn_weights in ["beta", "gamma", "moving_mean", "moving_variance"]:
        tf_name = "{}/head/tpu_batch_normalization/{}{}".format(model_name_tf, bn_weights, ema)
...
def keras_name_to_tf_name_block(
    keras_name, keras_block="block1a", tf_block="blocks_0", use_ema=True, model_name_tf="efficientnet-b0"
):
    ...
    if keras_block not in keras_name:
        raise ValueError("block name {} not found in {}".format(keras_block, keras_name))
...
def check_match(keras_block, tf_block, keras_weight_names, tf_weight_names, model_name_tf):
    ...
    if names_missing:
        raise ValueError("{} variables not found in checkpoint file: {}".format(len(names_missing), names_missing))

    names_unused = names_from_tf - names_from_keras
    if names_unused:
        warnings.warn("{} variables from checkpoint file are not used: {}".format(len(names_unused), names_unused))
...
if __name__ == "__main__":
    ...
    p.add_argument("--model", required=True, type=str, help="name of efficient model", choices=arg_to_model.keys())
    p.add_argument("--notop", action="store_true", help="do not include top layers", default=False)
    p.add_argument("--ckpt", required=True, type=str, help="checkpoint path")
    p.add_argument("--output", "-o", required=True, type=str, help="output (h5) file path")
    args = p.parse_args()
